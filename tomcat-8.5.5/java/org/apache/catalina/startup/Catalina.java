...
public class Catalina {
     ...
     */
    public void load() {
        // 1. 初始化JMX配置环境变量，tomcat使用JMX来管理各个组件
        initNaming();

        // 2. 创建Digester用于解析server.xml
        Digester digester = createStartDigester();

        // 3. 尝试从conf/server.xml加载
        File file = configFile();
        InputStream inputStream = new FileInputStream(file);
        // 4. 如果加载不到则加载server-embed.xml(在catalina.jar中)，否则直接return
        inputStream = getClass().getClassLoader().getResourceAsStream("server-embed.xml");

        // 5. 使用Digester解析server.xml，实例化各个组件，如Server、Container、Connector等
        InputSource inputSource = new InputSource(file.toURI().toURL().toString());
        inputSource.setByteStream(inputStream);
        digester.push(this);
        digester.parse(inputSource);

        // 6. 给Server设置Catalina相关信息
