...
public class NioEndpoint extends AbstractJsseEndpoint<NioChannel> {
    ...
    public void bind() throws Exception {
        // 实例化ServerSocketChannel，绑定端口和地址
        serverSock = ServerSocketChannel.open();
        socketProperties.setProperties(serverSock.socket());
        InetSocketAddress addr = (getAddress()!=null?new InetSocketAddress(getAddress(),getPort()):new InetSocketAddress(getPort()));
        // backlog用于配置TCP连接队列，不能及时accept的连接会先存在这个队列中，这里的backlog默认大小为100
        serverSock.socket().bind(addr,getBacklog());
        // 配置accept()方法在接收请求时是否需要阻塞
        serverSock.configureBlocking(true);
        // 配置读超时时间，默认20000毫秒
        serverSock.socket().setSoTimeout(getSocketProperties().getSoTimeout());

        // 初始化acceptor线程池和poller线程池的容量
        // Acceptor和Poller是Endpoint的内部类，都实现了Runnable接口
        // Acceptor的作用是负责处理accept()事件；Poller负责轮询selector的状态，查看是否有准备好的连接，有的话丢到线程池中处理
        if (acceptorThreadCount == 0) {
            // 这里有个官方注释：等待修复在多线程模式下性能不好的问题
